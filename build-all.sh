...
if make ${parallel} all-bfd TARGET-bfd=headers
then
...
# Build
cd "${bd_binutils}"
if make ${parallel} all-build all-binutils all-gas all-ld
then
...
# Install
cd "${bd_binutils}"
if make install-binutils install-gas install-ld 
then
...
# Install
cd "${bd_gcc_bs}"
if make install-gcc install-target-libgcc 
then
...
# Build
cd "${bd_gdb}"
if make ${parallel} all-build all-gdb all-sim 
then
...
# Install
cd "${bd_gdb}"
if make install-gdb install-sim 
then
